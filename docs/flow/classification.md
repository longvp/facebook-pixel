...
| T6 Prisma → MySQL + Pixel model | US-1,8 | **now** | Schema is fully specified; mechanical migration. |
| T7 AES-256-GCM token crypto | US-7 | **now** | Self-contained, unit-tested, full code in plan. |
| T8 Pixel data-access model | US-1..8 | **now** | CRUD + validation specified; unit-tested. |
| T9 CAPI event builder + sender | US-10 | **now** | Pure functions + fetch; unit-tested. |
| T10 Pixel list screen (Polaris) | US-1,2,5,6,7 | **now** | UI fully specified against mockup. |
| T11 Add pixel screen | US-3,7,8 | **now** | Form specified; shared component. |
| T12 Edit pixel screen | US-4,7,8 | **now** | Mirrors T11; Pixel ID immutable. |
| T13 orders/create webhook → Purchase | US-10 | **now** | Wires T8+T9; verified against dev store. |
| T14 Web Pixel Extension | US-9 | **complex** | Sandbox runtime, settings wiring (which pixels are active), event mapping, dedup. See complex-cases. |
| Tracking-pages picker (Selected/Excluded) | US-8 | **discuss** | UI/UX undecided. See questions.md. |
| Test event code persistence | US-7 | **discuss** | Store vs transient undecided. See questions.md. |

## Summary
- **now (7):** T6, T7, T8, T9, T10, T11, T12, T13 → ready for `/execute` (recommended order: T6 → T7 → T8 → T9 → T13 → T10 → T11 → T12).
- **complex (2):** T5 (scaffold — must run first, needs user), T14 (web pixel extension).
- **discuss (2):** tracking-pages picker, test-event-code persistence — resolve before they block T8/T10–T12 (currently stubbed: `trackingPages=[]`, behaves like ALL).
